defmodule ElixirPlaygroundWeb.UserAuthTest do
  use ElixirPlaygroundWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias ElixirPlayground.Accounts
  alias ElixirPlayground.Accounts.Scope
  alias ElixirPlaygroundWeb.UserAuth

  import ElixirPlayground.AccountsFixtures

  @remember_me_cookie "_elixir_playground_web_user_remember_me"
  @remember_me_cookie_max_age 60 * 60 * 24 * 14

  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, ElixirPlaygroundWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "log_in_user/3" do
    ...
    test "redirects to settings when user is already logged in", %{conn: conn, user: user} do
      ...
      assert redirected_to(conn) == ~p"/location-selector"
    end
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      ElixirPlaygroundWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        ElixirPlaygroundWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
